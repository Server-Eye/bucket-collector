...
/**
 * Calls bucketService.get() with the given bId, perform given reaction for each received message. 
    ...
    if (!reactor[type]) {
        ...
        buckets.get(bId).then(function (bucket) {
            ...
            if (bucket.messages.active.length > 0) {
                ...
                var nextMessage = getCallback(type, bucket, failedMessages, promises);
...
function getCallback(type, bucket, failedMessages, promises) {
    return function nextMessage(message) {
